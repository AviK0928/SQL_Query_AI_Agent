...
| P9 | Twice a pull request was merged before its last pushed commit was in it: PR #2 (merge commit, stale head; two verified commits missing) and PR #4 (squash, merged before the docs commit landed; this registry update missing). Both were caught by comparing `main`'s tree with the last verified commit, not by commit ancestry (which squash merges break), and fixed by a follow-up PR (#3, and the Phase 2 records PR). `main` keeps the resulting history rather than being force-pushed. Rule since: before merging, the PR's Commits tab must end at the hash the push cell printed. | Notebook cells P1-17, P1-18, P2-8 |
