...
| L14 | The repair prompt (`sql_repair@a5c30252`) has the schema but not the domain rules, so a repaired revenue query includes cancelled orders (policy P1). All four models failed r01, r08 and r10 for this reason. In production it affects only questions whose first query fails. Phase 7 experiment #1; the repair suite measures it directly. |
